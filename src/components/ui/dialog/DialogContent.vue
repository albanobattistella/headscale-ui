...
<template>
  <DialogPortal>
    ...
    <DialogContent
      ...
      :class="
        cn(
          'bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid max-h-[calc(100svh-2rem)] w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 overflow-x-hidden overflow-y-auto overscroll-contain rounded-lg border p-4 shadow-lg duration-200 sm:max-h-[calc(100svh-4rem)] sm:max-w-lg sm:p-6',
          props.class,
        )"
    >
      <slot />
